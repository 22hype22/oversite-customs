...
@credits_group.command(name="add", description="Gives credits to a member.")
...
@credits_group.command(name="remove", description="Takes credits from a member.")
...
@credits_group.command(name="view", description="Shows a member's credits and history.")
...
@bot.tree.command(name="payment", description="Creates a payment link through Stripe, a gamepass, or a shirt.")
...
@bot.tree.command(name="robuxlocker", description="Stocks the Robux Locker from the group's funds.")
...
@bot.tree.command(name="robuxlockerrate", description="Sets the Robux sell rate in USD per 1,000 Robux.")
...
@bot.tree.command(name="funds", description="Shows the group's available and pending Robux.")
@app_commands.describe(period="Which window to break down. This month if you leave it.")
...
@bot.tree.command(name="status", description="Shows the current order status.")
...
@bot.tree.command(name="portfolio", description="Posts your portfolio to its channel.")
...
PKGBACK_MAX_DIM = 2048  # downscale huge backgrounds so the stored config stays small
...
def _pkgback_prep(raw_bytes):
    """Normalize an uploaded background to PNG bytes, downscaling anything larger
    than PKGBACK_MAX_DIM so the base64 kept in config stays small. Returns the raw
    bytes unchanged if Pillow isn't available or the image can't be read."""
    try:
    ...
    try:
        ...
        w, h = img.size
        if max(w, h) > PKGBACK_MAX_DIM:
            r = PKGBACK_MAX_DIM / max(w, h)
            img = img.resize((max(1, round(w * r)), max(1, round(h * r))), Image.LANCZOS)
        out = io.BytesIO()
        img.save(out, "PNG")
        return out.getvalue()
    except Exception as e:
        print(f"[PackageBack] prep failed: {e}")
        return raw_bytes
...
@bot.tree.command(name="package", description="Posts a package card to a channel.")
...
async def package_cmd(interaction: discord.Interaction, channel: typing.Union[discord.TextChannel, discord.ForumChannel], tag: str = "", delivery: typing.Optional[discord.TextChannel] = None, payment: str = "", link: str = ""):
    ...
    if not comps:
        ...
        return
    # A forum tag is optional. When the forum itself insists on one and none
    # was picked, the post falls back to the forum's first tag when it posts.
    if isinstance(channel, discord.ForumChannel) and tag:
...
@bot.tree.command(name="orderlog", description="Logs an order with a quick form.")
...
@bot.tree.command(name="qualitycheck", description="Sends work in for a quality check with files and a form.")
...
@bot.tree.command(name="tax", description="Works out what you keep after the Roblox tax and what to charge.")
...
@bot.tree.command(name="pricing", description="Shows the pricing for a service.")
...
@bot.tree.command(name="setpricing", description="Sets your prices for a service.")
...
@bot.tree.command(name="joinsetup", description="Sets your pricing and creates your portfolio.")
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if cid == "ticket_select":
        ...
        parts = cid.split(":")  # pkg_claim:gp:{gpid}:{pkgmsg}:{deliverto}:{perk}
        await _pkg_claim_gamepass(interaction, parts[2], parts[3] if len(parts) > 3 else "", parts[4] if len(parts) > 4 else "", parts[5] if len(parts) > 5 else "")
    elif cid.startswith("pkg_claim:shirt:"):
        parts = cid.split(":")  # pkg_claim:shirt:{assetid}:{pkgmsg}:{deliverto}:{perk}
        await _pkg_claim_shirt(interaction, parts[2], parts[3] if len(parts) > 3 else "", parts[4] if len(parts) > 4 else "", parts[5] if len(parts) > 5 else "")
    elif cid.startswith("pkg_claim:stripe"):
        parts = cid.split(":")  # pkg_claim:stripe:{pkgmsg}:{deliverto}:{perk}
        await _pkg_claim_stripe(interaction, parts[2] if len(parts) > 2 else "", parts[3] if len(parts) > 3 else "", parts[4] if len(parts) > 4 else "")
    elif cid.startswith("pkg_claim:dp:"):
        parts = cid.split(":")  # pkg_claim:dp:{pkgmsg}:{deliverto}:{perk}
        await _pkg_claim_devproduct(interaction, parts[2] if len(parts) > 2 else "", parts[3] if len(parts) > 3 else "", parts[4] if len(parts) > 4 else "")
...
@bot.tree.command(name="feedback", description="Sends feedback to the team.")
async def feedback_cmd(interaction: discord.Interaction):
    await _pf_command(interaction, "customs-feedback")


@bot.tree.command(name="reportbug", description="Reports a bug to the team.")
...
@bot.tree.command(name="vouch", description="Leaves a vouch for a designer you bought from.")
...
@bot.tree.command(name="vouches", description="Shows a designer's rating and recent vouches.")
@app_commands.describe(designer="Whose vouches to show. Leave empty for yourself.")
...
@bot.tree.command(name="vouchremove", description="Removes a vouch.")
@app_commands.describe(designer="The designer the vouch is for", reviewer="Who left it. Staff can pick anyone, otherwise it's you.")
...
@bot.tree.command(name="sales", description="Shows each designer's orders, packages, and revenue.")
@app_commands.describe(period="How far back to count", designer="Only this designer, if you want.", public="Post it in the channel instead of just to you")
@app_commands.choices(period=[
    app_commands.Choice(name="This week", value="week"),
    app_commands.Choice(name="This month", value="month"),
...
@bot.tree.command(name="away", description="Marks you away, hides you from pricing, and tells your customers when you're back.")
@app_commands.describe(until="How long or until what date, like 3d, 2 weeks, 9/10, or sep 10.", note="Optional note shown to your customers")
...
@bot.tree.command(name="back", description="Ends your away mode early.")
...
@bot.tree.command(name="freerelease", description="Posts a free release that unlocks once enough people enter.")
...
@bot.tree.command(name="progress", description="Marks an order as in progress or completed.")
@app_commands.describe(stage="Where the order is now", note="Anything to add for the customer.")
...
@bot.tree.command(name="myorders", description="Shows your open and past orders.")
@app_commands.describe(member="Whose orders to show. Staff only.")
...
async def _pkg_deliver_receipt(interaction, pkg_msg_id, acct, price_str, product_url, deliver_to=None, perk_hint="", proof=None):
    ...
    try:
        ...
        elif perk_hint and perk_hint in ADS_PERK_KEYS and interaction.guild:
            # The perk key rode inside the Claim button's custom_id, so it
            # survives redeploys and other buyers clobbering the shared record.
            _ads_grant(interaction.guild.id, deliver_to or interaction.user.id, perk_hint)
            granted = perk_hint
            print(f"[Ads] granted {perk_hint} to {deliver_to or interaction.user.id} (from claim button)")
        else:
        ...
        if granted:
            # A purchase must survive an immediate crash or redeploy: snapshot
            # the inventory NOW instead of waiting out the debounced save.
            asyncio.create_task(_ads_flush_now(attempts=3))
    except Exception as e:
...
async def _pkg_run_flow(interaction, kind, pkg_msg_id, deliver_to, title=None, price_field=None, buy_url=None):
    ...
    # starting a different product on the same shop message) — the custom_id
    # can't, so the right perk is always granted.
    perk_key = _ads_perk_for_name(title or "") or ""
    if kind in ("devproduct", "gamepass"):
        await _pkg_flow_devproduct(interaction, title, price_field, pkg_msg_id, deliver_to, buy_url=buy_url, perk=perk_key)
    elif kind == "select":
        await _pkg_flow_select(interaction, price_field, pkg_msg_id, deliver_to, perk=perk_key)
    elif kind == "stripe":
        await _pkg_flow_stripe(interaction, price_field, pkg_msg_id, deliver_to, perk=perk_key)
    else:
...
async def _pkg_flow_devproduct(interaction, title, price_field, pkg_msg_id, deliver_to, buy_url=None, perk=""):
    ...
    view.add_item(discord.ui.Button(label="Claim Package", style=discord.ButtonStyle.success, custom_id=f"pkg_claim:dp:{pkg_msg_id}:{deliver_to}:{perk}"))
...
async def _pkg_flow_gamepass(interaction, title, pkg_msg_id, deliver_to, perk=""):
    ...
    view.add_item(discord.ui.Button(label="Claim Package", style=discord.ButtonStyle.success, custom_id=f"pkg_claim:gp:{gp['id']}:{pkg_msg_id}:{deliver_to}:{perk}"))
    await interaction.followup.send(embed=info_embed(
        "Your Gamepass",
...
async def _pkg_flow_stripe(interaction, price_field, pkg_msg_id, deliver_to, perk=""):
    ...
    view.add_item(discord.ui.Button(label="Claim Package", style=discord.ButtonStyle.success, custom_id=f"pkg_claim:stripe:{pkg_msg_id}:{deliver_to}:{perk}"))
    await interaction.followup.send(embed=info_embed(
...
async def _pkg_flow_select(interaction, price_field, pkg_msg_id, deliver_to, perk=""):
    ...
    view.add_item(discord.ui.Button(label="Claim Package", style=discord.ButtonStyle.success, custom_id=f"pkg_claim:shirt:{asset_id}:{pkg_msg_id}:{deliver_to}:{perk}"))
    await interaction.followup.send(embed=info_embed(
...
async def seed_secret_slots():
    ...
    slots = [{
        ...
        "description": ("The Roblox experience(s) where Purchase dev products are created. Paste "
                        "either a place ID (roblox.com/games/<ID>/…) OR an experience ID (the "
                        "number in the Creator Dashboard URL, create.roblox.com/dashboard/"
                        "creations/experiences/<ID>/overview) — both work. Have more than one "
                        "game? List several separated by commas — new items fill the first "
                        "experience, and once it's full they roll over into the next one "
                        "automatically. Each MUST be authorized for your Open Cloud API key (add "
                        "it to the key with developer-product write). Leave blank for the default "
                        "store."),
...
async def load_all_configs():
    ...
    features = [f for f in ("welcome", "invite", "tickets", "credits", "roblox-verify", "customs-giveaway", "customs-robux-locker", "customs-portfolio", "customs-packages", "customs-orderlog", "customs-infraction", "customs-promotion", "customs-qualitycheck", "customs-payment", "customs-logging", "customs-order-status", "customs-pricing", "music-addon", "auto-radio", "roblox-group-sync", "customs-messages", "customs-suggestions", "customs-feedback", "customs-reportbug", "customs-freerelease", "customs-blacklist", "customs-announce", "customs-smallui", "customs-vouches", "customs-sales", "invite-tracker", "marketplace", "ads", "customs-tts", "customs-gambling") if _base_allows_feature(f)]
